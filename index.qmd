...
def calculate_stats(username, repos, events, graphql_data):
    ...
    if graphql_data:
        ...
        # Process events for hourly data (GraphQL doesn't provide this)
        for event in events:
            ...
            if event["type"] == "PushEvent":
                ...
                stats["hourly_contributions"][hour] += 1
...
hero_html = f'''
<div class="hero">
    <h1>GITHUB INSIGHTS<span class="year">{YEAR}</span></h1>
    <a href="https://github.com/{GITHUB_USERNAME}" target="_blank" class="username">@{GITHUB_USERNAME}</a>
</div>
'''
...
# Determine coding time personality
time_periods = [
    ("Night", range(0, 6)),
    ("Morning", range(6, 12)),
...
time_data = {name: sum(hourly_data.get(h, 0) for h in hours) for name, hours in time_periods}
peak_time = max(time_data, key=time_data.get) if any(time_data.values()) else "Night"

time_reveals = {
    "Night": {"title": "NIGHT OWL", "subtitle": "The moon is your coding companion", "emoji": "🦉"},
    "Morning": {"title": "EARLY BIRD", "subtitle": "First commits before first coffee", "emoji": "🌅"},
    "Afternoon": {"title": "AFTERNOON WARRIOR", "subtitle": "Peak productivity hours", "emoji": "☀️"},
    "Evening": {"title": "EVENING CODER", "subtitle": "Golden hour, golden commits", "emoji": "🌆"}
}
reveal = time_reveals.get(peak_time, time_reveals["Night"])

personality_html = f'''
<h2 class="section-header">YOUR DEVELOPER PERSONALITY</h2>
<div class="personality-duo">
    ...
    <div class="personality-card time-card">
        <div class="card-label">You're a...</div>
        <div class="card-emoji">{reveal["emoji"]}</div>
        <div class="card-title">{reveal["title"]}</div>
        <div class="card-subtitle">{reveal["subtitle"]}</div>
    </div>
</div>
'''
...
for month in months_order:
    ...
    height = (value / max_monthly) * 160 if max_monthly > 0 else 0
...
# Find peak day
peak_day = max(daily_data, key=daily_data.get) if any(daily_data.values()) else None
...
for day in days_order:
    ...
    is_peak = " peak-day" if day == peak_day and value > 0 else ""
    heatmap_html += f'''
        <div class="heatmap-row">
            <div class="heatmap-label">{day}</div>
            <div class="heatmap-cell heat-{heat}{is_peak}">{value:,}</div>
        </div>
    '''
...
if peak_day and daily_data.get(peak_day, 0) > 0:
    heatmap_html += f'<p style="text-align: center; color: var(--gh-green-light); margin-top: 1rem; font-size: 1.1rem;">You code most on <strong>{peak_day}s</strong> with {daily_data[peak_day]:,} contributions!</p>'
...
<div class="fun-facts">
    <div class="fun-fact">
        You squashed approximately <span class="highlight">{bugs_squashed:,}</span> bugs this year
    </div>
    <div class="fun-fact">
...
footer_html = '''
<div class="footer">
    <div class="tagline">Here's to another year of commits.</div>
    <a href="https://github.com/isfarbaset/github-insights" target="_blank" class="github-btn" title="View on GitHub">
